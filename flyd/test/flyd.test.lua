...
local function flyd_put(conn, t)
	return conn:call('flyd_new_flight', t)
end
...
test:test('flyd.put.get', function(test)
	...
	test:test('flyd.put - arguments', function(test)
		...
		local ret = flyd_put(conn, 'non table')
		test:isnt(ret[1][1], true, 'put: non table arg')

		ret = flyd_put(conn, {})
		test:is(ret[1][1], true, 'put: empty table arg')

		ret = flyd_put(conn, {{}, 'non table'})
		test:is(ret[1][1], true, 'put: non empty table arg')

		ret = flyd_put(conn, {{ 1, 2, 3 }})
		test:is(ret[1][1], true, 'put: invlalid args count')

		ret = flyd_put(conn, {{ 'a', 'b', 'c', 'd', 'e' }})
		test:is(ret[1][1], true, 'put: invlalid args')
	end)
	...
	test:test('flyd.get - arguments', function(test)
		...
		test:is(ret[1][1], 'null', 'ret val is null')

		ret = flyd_get(conn, {1, 2, 'a'}, 'ru')
		test:is_deeply(ret[1], { 'null', 'null', 'null' }, 'table of non tables')

		ret = flyd_get(conn, {{ 1, 2, 3 }}, 'en')
		test:is(ret[1][1], 'null', 'invalid args cnt')

		ret = flyd_get(conn, {{ 1, 2, 3, 4, 5, 6 }}, 'us')
		test:is(ret[1][1], 'null', 'invalid locale')

		ret = flyd_get(conn, {{ '1', '2', '3', 4, 5, 6 }}, 'en')
		test:is(ret[1][1], 'null', 'invalid args')

		ret = flyd_get(conn, {{ '1', '2', 3, 4, 5, 6 }}, 'en')
		test:is(ret[1][1], 'null', 'not found')
	end)

	test:test('flyd.put.get', function(test)
		test:plan(15)

		flyd_put(conn, {{ 'AA', 100, 20, 7, 2015 }})
		fiber.sleep(1) -- wait for `put' complete

		local ret = flyd_get(conn, {{ 'CODE', 'AA', 100, 20, 7, 2015 }}, 'en')
		test:isnt(ret[1][1], 'null', 'insert + select success')

		local data = json.decode(ret[1][1])
		...
		flyd_put(conn, {{ 'ERR', 100, 20, 7, 2015 }})
		fiber.sleep(1)

		ret = flyd_get(conn, {{ 'CODE', 'ERR', 100, 20, 7, 2015 }}, 'en')
		test:is(ret[1][1], 'null', 'check for error select')

		flyd_put(conn, {{ 'ZR', 100, 20, 7, 2015 }})
		fiber.sleep(1)

		ret = flyd_get(conn, {{ 'CODE', 'ZR', 100, 20, 7, 2015 }}, 'en')
		data = json.decode(ret[1][1])
		...
		flyd_put(conn, {{ 'CB', 100, 20, 7, 2015 }})
		fiber.sleep(1)
		...
		flyd_put(conn, {{ 'ERR_JSON', 100, 20, 7, 2015 }})
		fiber.sleep(1)
		...
		flyd_put(conn, {{ 'ERR_IN_JSON', 100, 20, 7, 2015 }})
		fiber.sleep(1)
...
test:test('flyd.update', function(test)
	...
	test:test('flyd.update.delay', function(test)
		...
		test:isnt(ret[1][1], 'null', 'check for new record')
	end)
	...
	test:test('flyd.update.cancel', function(test)
		...
		test:is(ret[1][1], 'null', 'check for update after cancel')
	end)
end)
test:check()
